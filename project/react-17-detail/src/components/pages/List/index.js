...
class List extends Component {
    ...
    getGoods () {
        ...
        axios.get ('/zol/index.php?c=Shop_IndexV2&a=AjaxGetGoods&pos=2', {
            params: { page }
        }).then (({data}) => {
            if ( !this.mounted ) return false;
            let goods = this.state.goods.concat(data.info.data)
            let hasMore = data.isEnd === '0'
            this.page ++;           
    ...
    render () {
        let row = (data) => {
            return <GoodItem data = { data } key = { data.goods_id }/>
        }
