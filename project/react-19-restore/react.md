...
4. react-redux中的connect方法的返回值是一个:  
    let HOC = connect()
    ContainerComponent = HOC(UIComponent)
    对象、(容器组件)、UI组件、数组
...
8. 下面哪些方法可以使componentWillUpdate执行
    ...
    属性更改或者状态更改,并且shouldComponentUpdate返回true
...
13. 请列举你所了解的react中的性能优化
    ...
    * 没必要存在store中的数据，存在组件的state中就可以
    * 函数的this指向放在constructor中改变
...
13.1 pureComponnet
    ...
    import React, {pureComponeng} from 'react
